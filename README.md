...
Data=pd.read_csv(r"data/1YearStorage.out", sep=r'\s+', usecols=['K', 'Inj', 'Por', 'b', 'CHD', 'Ext', 'DSP','REN_3_2'])
...
There is no missing data and all features and target are continuous. So, no data processing is needed to handle missing data or categorical features. One of the many qualities of Decision Trees is that they do not need much data preparation like feature scaling at all. We just need to make a new feature as the ratio of extraction to injection rate.
...
## Decision Tree (DT)
DTs are the fundamental components of Random Forests (RF) and other ensemble methods explored here. DTs can be used for both classification and regression tasks. The Scikit-learn documentation serves as an excellent resource for comprehensively understanding the parameters associated with each of the algorithms in the Scikit-learn library. [This page](https://scikit-learn.org/stable/modules/generated/sklearn.tree.DecisionTreeRegressor.html) presents the parameters for DecisionTreeRegressor we need for our regression problem. 

DTs are nonparametric models because the number of their parameters is not determined prior to training (unlike a parametric model like linear regression which assumes the target variable is modeled as a linear combination of the features). If left unconstrained, DTs will adapt themselves to the training data and produce an overfitted model. To avoid this, we need to restrict the DTs freedom during training. Below, we show an example in which we limit the maximum depth of our DTs. By default, 'max_depth' is 'None' meaning nodes are expanded until all leaves are pure or until all leaves contain less than 'min_samples_split' samples. Here, we also increase the 
...
The "value" in each node is the average target value of samples in the node. The main weakness of DTs is that they are very sensitive to small variations in the training data. To address this weakness, we have to use Random Forest or other Ensemble methods. 

## Ensemble Learning
Ensemble learning means training a group of models/predictors and then making predictions using all of them. For classification problems, the predicted class can be the one which has the majority of votes from the ensemble models. For regression problems, like our problem, the predicted REN is the average of REN obtained from all Ensemble models. 

Ensemble methods work best when the predictors are as independent from one another as possible. 

### Ensemble of different algorithms 
This ensemble method uses predictors with different algorithms. For example, using DTs, SVD or linear regression in the ensemble and aggregate their results (using average of models as the prediction of the ensemble). A major benefit is that different models can be trained in parallel.

### Bagging
This method uses the same algorithm for all predictors but training them on different random subset of training set (rows) or subset of features (columns). Again, training models can be performed in parallel. Here, we explore the major hyperparameters of [BaggingRegressor](https://scikit-learn.org/stable/modules/generated/sklearn.ensemble.BaggingRegressor.html) API of scikit-learn. 
...
-  bootstrap: it is a boolean. If True (which is default), samples are drawn with replacement (which is called bagging short for bootstrap aggregating). If False, sampling without replacement (also called pasting) is performed meaning a sample (row) can only be selected from X once. 
...
0.019
```
We can see that using an ensemble of 50 DTs, we are able to reduce MSE from 0.035 (from the best single DT model) to 0.019.  

### Random Forest (RF)
A Random Forest is essentially a specialized form of Bagging, as described above, with the key difference being that the base estimator/algorithm used in a Random Forest is always a decision tree. The scikit-learn [RF API](https://scikit-learn.org/stable/modules/generated/sklearn.ensemble.RandomForestRegressor.html) has all hyperparameters of Decision trees plus all the hyperparameters of 'BaggingRegressor'. Below we build a RF model with similar parameters to the Bagging model we created earlier. 
...
Results show that background groundwater gradient (CHD) and Hydraulic conductivity (K) are the most important features for REN prediction. 

### Gradient Boosting
Boosting refers to combining several weaker predictors into a strong predictor by training predictors sequentially, each trying to correct its predecessor. Gradient boosting tries fitting a new predictor to the residual errors made by the previous predictor. 

```python
lr = 0.1  # learning rate (shrinkage): scales each tree's contribution to prevent overfitting
...
M1_residuals=y_train - lr * Model1.predict(X_train)
...
M2_residuals=M1_residuals - lr * Model2.predict(X_train)
Model3.fit(X_train, M2_residuals)

#make prediction: sum contributions of all trees, each scaled by learning rate
y_pred = lr*Model1.predict(X_test) + lr*Model2.predict(X_test) + lr*Model3.predict(X_test)
GB_MSE=mean_squared_error(y_pred,y_test)
print(GB_MSE)

```

[Here](https://scikit-learn.org/stable/modules/generated/sklearn.ensemble.GradientBoostingRegressor.html) is the scikit API for regression gradient boosting.  
...
0.006
```
In order to find the optimal number of models in our ensemble (parameter 'n_estimators'), we can use 'staged_predict()' method which returns y_pred for each sequential model.
...
The plot shows that 20 can be a reasonable value for the number of models in our Gradient Boosting ensemble. Instead of scikit API, we can also use xgboost which provides an optimized implementation
```python
import xgboost
...
`feature_importances_` from Random Forest tells you *which* features matter globally but not *how* they affect individual predictions. SHAP assigns each prediction a contribution score per feature, grounded in cooperative game theory (Lundberg & Lee, 2017). A SHAP summary plot shows both direction (positive/negative effect on REN) and magnitude for every sample — more informative than a bar chart of global importances.
...
All models above use a single 90/10 train-test split (`random_state=42`). For a dataset of this size, a different seed can shift MSE noticeably — making direct model comparisons less reliable. The standard remedy is **k-fold cross-validation**, which averages performance across k different splits. Cross-validation is not implemented here because the focus of this tutorial is on understanding the ensemble algorithms themselves, not on validation strategy.
